...
class MobileDetect
{
    ...
    public function __construct(var cache = null, array config = [])
    {
        if cache == null {
            let this->cache = new Cache();
        } else {
    ...
    {
        var knownHttpHeaders, httpHeaders, headerName;
        let knownHttpHeaders = array_merge(
        ...
        let httpHeaders = [];
        for headerName in knownHttpHeaders {
            if isset _SERVER[headerName] {
                let httpHeaders[headerName] = _SERVER[headerName];
            }
        }
    ...
    {
        var userAgent, altHeader;
        let this->httpHeaders = httpHeaders;
        if count(httpHeaders) == 0 {
            return;
        }
        let userAgent = "";
        for altHeader in this->getUaHttpHeaders() {
            if !empty(this->httpHeaders[altHeader]) {
                let userAgent .= this->httpHeaders[altHeader] . " ";
            }
        }
        if !empty(userAgent) {
    ...
    public function getHttpHeader(string header) -> string | null
    {
        var altHeader;
        if !memstr(header, "_") {
            let header = str_replace("-", "_", header);
            let header = strtoupper(header);
        }
        let altHeader = "HTTP_" . header;
    ...
    {
        return self::knownMobilePositiveHeaders;
    }

    public function getUaHttpHeaders() -> array
    {
        return self::knownUserAgentHttpHeaders;
    }

    public function getCloudFrontHttpHeaders() -> array
    {
        return self::knownCloudFrontHeaders;
    }

    private function prepareUserAgent(string userAgent) -> string
    {
        let userAgent = trim(userAgent);
        return substr(userAgent, 0, (int)this->config["maximumUserAgentLength"]);
    }

    public function setUserAgent(string userAgent) -> string
    {
        var preparedUserAgent;
        let preparedUserAgent = this->prepareUserAgent(userAgent);
        let this->userAgent = preparedUserAgent;
    ...
    {
        return typeof this->userAgent == "string";
    }
    ...
    {
        var mobileHeader, matchType, _match;
        for mobileHeader, matchType in this->getMobileHeaders() {
            if isset this->httpHeaders[mobileHeader] {
                if isset matchType["matches"] && typeof matchType["matches"] == "array" {
                    for _match in matchType["matches"] {
                        if memstr(this->httpHeaders[mobileHeader], _match) {
                            return true;
                        }
                    }
                    return false;
    ...
    public function __call(string name, array arguments)
    {
        ...
        let ruleName = substr(name, 2);
        return this->is(ruleName);
    }

    public function isMobile() -> bool
    {
        var cacheKey, cacheItem, result;
        if !this->hasUserAgent() {
        ...
        try {
            let cacheKey = this->createCacheKey("mobile");
            ...
            if this->hasHttpHeaders() && this->checkHttpHeadersForMobile() {
                ...
                let result = this->matchUserAgentWithFirstFoundMatchingRule();
    ...
    {
        var cacheKey, cacheItem, _regex, regexString;
        if !this->hasUserAgent() {
        ...
        try {
            let cacheKey = this->createCacheKey("tablet");
            let cacheItem = this->cache->get(cacheKey);
            if cacheItem !== null {
                return cacheItem;
            }
            ...
            for _regex in self::getTabletDevices() {
                let regexString = _regex;
                if typeof _regex == "array" {
                    let regexString = implode("|", _regex);
                }
    ...
    public function is(string ruleName) -> bool
    {
        ...
        try {
            let cacheKey = this->createCacheKey(ruleName);
            let cacheItem = this->cache->get(cacheKey);
            if cacheItem !== null {
                return cacheItem;
            }
            let result = this->matchUserAgentWithRule(ruleName);
            this->cache->set(cacheKey, result, this->config["cacheTtl"]);
            return result;
        } catch CacheInvalidArgumentException, e {
    ...
    {
        var match, matches;
        let match = preg_match("#" . regex . "#is", userAgent, matches);
        if match {
            let this->matchingRegex = regex;
            let this->matchesArray = matches;
        }
        return (bool) match;
    }
    ...
    {
        var _regex, regexString;
        for _regex in this->getRules() {
            if empty(_regex) {
                continue;
            }
            if typeof _regex == "array" {
                for regexString in _regex {
                    if this->match(regexString, this->getUserAgent()) {
                        return true;
                    }
                }
    ...
    {
        var result, _rules, regexString;
        let result = false;
        let ruleName = strtolower(ruleName);
        let _rules = array_change_key_case(this->getRules());
        if !empty(_rules[ruleName]) {
            let regexString = _rules[ruleName];
            if typeof _rules[ruleName] == "array" {
                let regexString = implode("|", _rules[ruleName]);
            }
            let result = this->match(regexString, this->getUserAgent());
        }
        return result;
    }
    ...
    {
        var arrVer;
        let ver = str_replace(["_", " ", "/"], ".", ver);
        let arrVer = explode(".", ver, 2);
        if isset arrVer[1] {
    ...
    {
        var properties, propertyMatchString, propertyPattern, match;
        if empty(propertyName) || !this->hasUserAgent() {
            return false;
        }
        if type !== self::VERSION_TYPE_STRING && type !== self::VERSION_TYPE_FLOAT {
            let type = self::VERSION_TYPE_STRING;
        }
        let properties = self::getProperties();
        if isset properties[propertyName] {
            let properties[propertyName] = (array) properties[propertyName];
            for propertyMatchString in properties[propertyName] {
                let propertyPattern = str_replace("[VER]", self::VERSION_REGEX, propertyMatchString);
    ...
    {
        var userAgentKey, httpHeadersKey, cacheKey, cacheKeyFn;
        let userAgentKey = this->hasUserAgent() ? this->userAgent : "";
        let httpHeadersKey = this->hasHttpHeaders() ? self::flattenHeaders(this->httpHeaders) : "";
        let cacheKey = key . ":" . userAgentKey . ":" . httpHeadersKey;
        let cacheKeyFn = this->config["cacheKeyFn"];
