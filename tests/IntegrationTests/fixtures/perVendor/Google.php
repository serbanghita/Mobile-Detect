<?php
return array(
    array(
		'ua' => 'Mozilla/5.0 (Linux; U; Android 2.2; en-us; Nexus One Build/FRF91) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1',
        'isMobile' => true,
        'isTablet' => false,
        'browser' => 'Safari Mobile',
        'browserVersion' => '4.0',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '2.2',
        'model' => 'Nexus One',
        'modelVersion' => null
    ),
	array(
        'ua' => 'Mozilla/5.0 (Linux; Android 4.2.2; Nexus 4 Build/JDQ39) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.40 Mobile Safari/537.31 OPR/14.0.1074.54070',
        'isMobile' => true,
        'isTablet' => false,
        'browser' => 'Opera Mobi',
        'browserVersion' => '14.0.1074.54070',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.2.2',
        'model' => 'Nexus 4',
        'modelVersion' => '4'
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; Android 4.2.2; Nexus 4 Build/JDQ39) AppleWebKit/537.31 (KHTML, like Gecko) Chrome/26.0.1410.58 Mobile Safari/537.31',
        'isMobile' => true,
        'isTablet' => false,
        'browser' => 'Chrome Mobile',
        'browserVersion' => '26.0.1410.58',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.2.2',
        'model' => 'Nexus 4',
        'modelVersion' => '4'
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; U; Android 4.1.1; en-us; Google Nexus 4 - 4.1.1 - API 16 - 768x1280 Build/JRO03S) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30',
        'isMobile' => true,
        'isTablet' => false,
        'browser' => 'Safari Mobile',
        'browserVersion' => '4.0',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.1.1',
        'model' => 'Nexus 4',
        'modelVersion' => '4'
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; U; Android 4.1.1; en-us; Google Galaxy Nexus - 4.1.1 - API 16 - 720x1280 Build/JRO03S) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30',
        'isMobile' => true,
        'isTablet' => false,
        'browser' => 'Safari Mobile',
        'browserVersion' => '4.0',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.1.1',
        'model' => 'Galaxy Nexus',
        'modelVersion' => null
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; Android 4.1.1; Nexus 7 Build/JRO03D) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.166  Safari/535.19',
        'isMobile' => true,
        'isTablet' => true,
        'browser' => 'Chrome Mobile',
        'browserVersion' => '18.0.1025.166',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.1.1',
        'model' => 'Nexus 7',
        'modelVersion' => '7'
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; Android 4.2; Nexus 7 Build/JOP40C) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.166 Safari/535.19',
        'isMobile' => true,
        'isTablet' => true,
        'browser' => 'Chrome Mobile',
        'browserVersion' => '18.0.1025.166',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.2',
        'model' => 'Nexus 7',
        'modelVersion' => '7'
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; Android 4.1.2; Nexus 7 Build/JZ054K) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.166 Safari/535.19',
        'isMobile' => true,
        'isTablet' => true,
        'browser' => 'Chrome Mobile',
        'browserVersion' => '18.0.1025.166',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.1.2',
        'model' => 'Nexus 7',
        'modelVersion' => '7'
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; U; Android 4.1.2; cs-cz; Nexus S Build/JZO54K; CyanogenMod-10.0.0) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30',
        'isMobile' => true,
        'isTablet' => false,
        'browser' => 'Safari Mobile',
        'browserVersion' => '4.0',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.1.2',
        'model' => 'Nexus S',
        'modelVersion' => null
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; Android 4.3; Nexus 10 Build/JWR66Y) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/29.0.1547.72 Safari/537.36',
        'isMobile' => true,
        'isTablet' => true,
        'browser' => 'Chrome Mobile',
        'browserVersion' => '29.0.1547.72',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => '4.3',
        'model' => 'Nexus 10',
        'modelVersion' => '10'
    ),

    array(
        'ua' => 'Mozilla/5.0 (Linux; U; Android; en_us; Nexus 7 Build/) AppleWebKit/530.17 (KHTML, like Gecko) Version/4.0 NetFrontLifeBrowser/2.3 Mobile (Dragonfruit)',
        'isMobile' => true,
        'isTablet' => true,
        'browser' => 'NetFront',
        'browserVersion' => '2.3',
        'operatingSystem' => 'Android',
        'operatingSystemVersion' => null,
        'model' => 'Nexus 7',
        'modelVersion' => null
    ),
);
